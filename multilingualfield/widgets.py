...
class CustomClearableFileInput(ClearableFileInput):
    ...
    """
    template_with_initial = (u'%(initial_file)s %(initial_text)s: '
    ...
    def render(self, name, value, attrs=None):
        ...
        template = u'%(input)s'
        substitutions[u'input'] = super(ClearableFileInput, self).render(name, value, attrs)

        if value and hasattr(value, u'url'):
            template = self.template_with_initial
            substitutions[u'initial'] = format_html(u'<a href="{0}">{1}</a>', value.url, force_text(value))
            initial_filename_name = self.initial_filename_name(name)
            substitutions[u'initial_file'] = HiddenInput().render(initial_filename_name, value,
                                                                  attrs={u'id': initial_filename_name})
            if not self.is_required:
                checkbox_name = self.clear_checkbox_name(name)
                checkbox_id = self.clear_checkbox_id(checkbox_name)
                substitutions[u'clear_checkbox_name'] = conditional_escape(checkbox_name)
                substitutions[u'clear_checkbox_id'] = conditional_escape(checkbox_id)
                substitutions[u'clear'] = CheckboxInput().render(checkbox_name, False, attrs={u'id': checkbox_id})
    ...
    def value_from_datadict(self, data, files, name):
        ...
        if not self.is_required and CheckboxInput().value_from_datadict(
            ...
            # If the user contradicts themselves (uploads a new file AND checks the "clear" checkbox), we return a
            # unique marker object that FileField will turn into a ValidationError.
            # False signals to clear any existing value, as opposed to just None
            return FILE_INPUT_CONTRADICTION if upload else False  # FIXME undefined FILE_INPUT_CONTRADICTION
        # Return the uploaded file or figure out the name of the hidden field which holds the existant filename
        # (if one exists)  If that hidden field is in the widget, pass the file path of that file on, defaults to None
